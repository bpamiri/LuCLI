...
## Unreleased
- **Fix: Profile-Aware Module Commands:** `lucli modules list`, `modules init`, `modules run`, and `modules uninstall` now resolve the modules directory via `LucliPaths.resolve().modulesDir()` instead of duplicating path-resolution with a hardcoded `~/.lucli` fallback. Under the Wheels profile (`wheels` binary), these commands now correctly target `~/.wheels/modules/` — previously they silently operated on `~/.lucli/modules/` even when `wheels system paths` reported the correct `~/.wheels/modules` location. The `modules list` heading also now reflects the active profile's display name (e.g. `Wheels Modules` when invoked as `wheels`). `ModuleRepositoryIndex` `settings.json` lookup was routed through the same resolver. Adds regression tests for both the list command output and the repository-index resolution.
