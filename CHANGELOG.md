...
## Unreleased
- **Fix: Default `rewrite.config` Now Actually Serves Static Files:** The bundled `rewrite_template/rewrite.config` was using a stack of `RewriteCond %{REQUEST_URI} !pattern` entries before a single rewrite `RewriteRule` to "skip" static-file paths. On Tomcat 11's `RewriteValve` that idiom does NOT short-circuit the way Apache `mod_rewrite` does — the conditions effectively don't gate the rule, the rule fires for every request, and `public/` static files (`.css`, `.js`, `.txt`, `.png`, etc.) silently 404 even though their extensions are explicitly listed in the negation. The template now expresses the same intent as positive-match `RewriteRule … - [L]` passthroughs (extension match, conventional static directories, `/lucee/`, `/rest/`), which terminate rule processing for matching requests and let Tomcat's default servlet serve the file. Net effect: `public/assets/foo.css`, `public/__sentinel.txt`, etc. now return HTTP 200 with `Content-Type` set correctly; framework routing through `index.cfm` still works for `/`, `/posts`, etc. Discovered during a Wheels Tutorial fresh-VM bake (the bonus-basecoat chapter's `curl -o public/assets/basecoat.min.css ...` instructions were dead on arrival because the file existed but the rewrite intercepted the request before the default servlet could serve it). Verified end-to-end on Tomcat 11.0.13 + Lucee Express 7.0.0.395.
