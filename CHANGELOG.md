...
## Unreleased
- **Fix: Global Flags Survive Module-Shortcut Re-Dispatch:** `lucli <module> <args> --verbose` (and the binary-name-routed form, e.g. `wheels test --verbose`) now delivers the verbose signal to the module. The root command consumes the global flags (`-v/--verbose`, `-d/--debug`, `-t/--timing`, `-w/--whitespace`) wherever they appear, then re-dispatches through `spec.commandLine().execute("modules", "run", ...)` — and that nested re-parse reset the option fields to their defaults, so `LuCLI.verbose` (and the `verboseEnabled` init arg every module receives) was `false` by the time the module executed. The consumed flags are now re-asserted on the re-dispatched command line (module shortcuts and file-run shortcuts both). Fixes the launcher half of wheels-dev/wheels#3113.
